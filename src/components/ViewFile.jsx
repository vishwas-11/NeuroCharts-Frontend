...
//   if (error) {
//     return (
//       <div className="bg-gray-800 p-6 rounded-lg shadow-xl border border-red-700 text-center">
...
//                 <th
//                   key={index}
//                   className="px-6 py-3 border-b-2 border-gray-600 bg-gray-900 text-left text-sm font-semibold uppercase tracking-wider"
//                 >
//                   {header}
//                 </th>
//               ))}
...
//               <tr key={rowIndex} className="hover:bg-gray-600 transition-colors duration-150">
...
// /*
//   Explanation for src/components/ViewFile.jsx:
//   - `useParams`: A hook from `react-router-dom` that extracts the `id` from the URL.
//   - `useEffect`: Calls the new `authService.getExcelDataById` to fetch the file's data from the backend using the ID.
//   - State variables (`fileData`, `isLoading`, `error`) manage the component's state.
//   - Conditional rendering handles loading, error, and no-data states, with buttons to navigate back.
//   - A responsive table displays the data. It uses the `headers` from the fetched data to create the table headers and then maps through the `data` to create the table rows and cells.
//   - `navigate(-1)` is a convenient way to go back to the previous page in the history, which will be the dashboard.
// */
